 ...
 * Sela Civil Advisory Ltd - Roles & Responsibilities in the NZGTTM Ecosystem course (standalone web version)
 *
 * The Firebase/Firestore result-recording integration is loaded with a
 * dynamic import() inside recordResultOnceIfPassed() below, rather than a
 * static top-level import. A static import would force this whole file to
 * be loaded as an ES module - and if that import ever fails (the page is
 * opened directly as a local file:// preview, the site's firebase-init.js
 * 404s, etc.), the ENTIRE script silently fails to run, breaking navigation,
 * the quiz and everything else, not just the Firestore write. Loading it
 * lazily and dynamically means the whole course still works standalone
 * (including local file:// previewing with no LMS/Firebase available at
 * all) and only the "send result to head office" step degrades gracefully
 * if Firebase can't be reached.
 */
(function () {
  "use strict";
  ...
  // Empty until narration is recorded for this course - add ids here (matching
  // screen.id / "quiz-qN") once the corresponding mp3 is added to assets/audio/
  // and listed in imsmanifest.xml. The narration bar stays hidden while this is empty.
  ...
  function init() {
    ...
    els.progressTrack = qs("#progressTrack");
    els.courseFooter = qs("#courseFooter");
    els.startGate = qs("#startGate");
    els.startGateIcon = qs("#startGateIcon");
    els.startGateEyebrow = qs("#startGateEyebrow");
    els.startGateTitle = qs("#startGateTitle");
    els.startGateSubtitle = qs("#startGateSubtitle");
    els.startGateMeta = qs("#startGateMeta");
    els.startGateBtn = qs("#startGateBtn");
  ...
  // Shows a click-to-begin splash instead of rendering the first screen
  // straight away. This matters now that most topics open with a video:
  // browsers only allow audio/video with sound to auto-play once the
  // visitor has interacted with the page, so renderScreen() (which
  // triggers the Welcome video's autoplay) is deferred until this button's
  // click handler calls it - that click is the interaction the browser is
  // waiting for, so autoplay then works on the very first video too, not
  // just on subsequent Next/Back clicks.
  function setupStartGate() {
    var hasProgress = state.screenIndex > 0 || state.quizScore !== null;

    if (els.startGateIcon && COURSE.screens[1] && COURSE.screens[1].icon) {
      els.startGateIcon.innerHTML = COURSE.screens[1].icon;
    }
    els.startGateTitle.textContent = COURSE.meta.title;
    els.startGateSubtitle.textContent = COURSE.meta.subtitle || "";
    var metaBits = [];
    if (COURSE.meta.duration) metaBits.push("Approx. " + COURSE.meta.duration);
    metaBits.push("includes short topic videos and a knowledge check");
    els.startGateMeta.textContent = metaBits.join(" · ");
    els.startGateBtn.textContent = hasProgress ? "Resume course" : "Start course";
    if (hasProgress) {
      els.startGateEyebrow.textContent = "Welcome back";
    }

    els.startGateBtn.addEventListener("click", function () {
      els.progressTrack.style.display = "";
      els.courseFooter.style.display = "";
      renderScreen();
    });
  }
  ...
  // Renders a topic video. The Next button stays disabled until the video's
  // "ended" event fires, so the learner has to watch the topic through
  // before moving on to its recap-and-reflect slide. Re-visiting the screen
  // (e.g. via Back then Next) always re-renders a fresh <video> element, so
  // the gate re-applies each time - the same "must engage every visit"
  // behaviour as the dwell timer on content screens.
  function renderVideoScreen(screen) {
    var slide = document.createElement("div");
    slide.className = "slide video-slide";
    var html = "";
    html += "<div class=\"eyebrow\">" + screen.eyebrow + "</div>";
    html += "<h1>" + screen.title + "</h1>";
    html += "<div class=\"video-wrap\"><video id=\"screenVideo\" class=\"screen-video\" controls playsinline preload=\"auto\" src=\"" + screen.videoSrc + "\"></video></div>";
    slide.innerHTML = html;
    els.body.appendChild(slide);

    setNarrationForScreen(null); // narration audio is baked into the video itself

    var videoEl = qs("#screenVideo");
    els.nextBtn.textContent = "Next";
    els.nextBtn.disabled = true;
    els.waitHint.textContent = "Watch the video to continue";
    ...
    // Reaching this screen always follows a user gesture (the start-gate
    // button on the very first screen, or a Next/Back click on every screen
    // after that), so autoplay-with-sound is allowed here.
    var playPromise = videoEl.play();
    if (playPromise && typeof playPromise.catch === "function") {
      playPromise.catch(function () {
        els.waitHint.textContent = "Press play to watch the video, then continue";
      });
    }
  }
  ...
  //
  // Loads the Firebase integration with a dynamic import() rather than a
  // static one (see the file-header comment) - if this course is being
  // previewed locally, or the site's js/firebase-init.js is unreachable
  // for any reason, the import rejects, we fall into the .catch() below,
  // and the rest of the course (already fully working at this point) is
  // completely unaffected.
  function recordResultOnceIfPassed() {
    if (!state.quizPassed) return;

    var FLAG_KEY = "sela_result_recorded::nzgttm-roles-01::v1";
    try {
    ...
    import("../../js/firebase-init.js").then(function (fb) {
      ...
      return fb.addDoc(fb.collection(fb.db, "submissions"), {
        userId: user.uid,
        email: (user.email || "").toLowerCase(),
        courseId: "nzgttm-roles-01",
        courseCode: "NZGTTM-ROLES-01",
        courseTitle: "TTM Roles & Responsibilities",
